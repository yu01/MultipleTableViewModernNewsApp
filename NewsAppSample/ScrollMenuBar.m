...
-(id)initWithArray:(NSArray *)array point:(CGPoint)point {
    ...
    CGRect frame = CGRectMake(point.x, point.y , _screenWidth, VIEW_HEIGHT) ;
...
-(void)initScrollView{
    CGRect frame = CGRectMake(0, 0, _screenWidth, VIEW_HEIGHT) ;
...
-(void)tabTaped:(UITapGestureRecognizer *)recognizer{
    ...
    _selectedIndex = [self indexWithTag:tab.tag];
    ...
    // オフセット計算
    CGFloat offsetX = tab.frame.origin.x + tab.frame.size.width/2 - _screenWidth/2 ;
...
//    NSLog(@"offsetX:%f",offsetX) ;
    if(offsetX <= 0){
        offsetX = 0 ;
    } else if (offsetX >= _scrollView.contentSize.width - _screenWidth ){
        offsetX = _scrollView.contentSize.width - _screenWidth ;
    }
